...
- `LcpArray` (`src/lcp.rs`): Kasai LCP over the SA rows stored capped at `u16::MAX`, with
  ...
  resident; opt-in via `FmIndexConfig::build_lcp` (default `false`). A `BidirFmIndex`
  builds one per half (the forward one backs `contract_left` / `parent_fwd`, the reverse
  one `contract_right`), so opting in grows its resident and serialized size by ~2.7
  B/base per half. Default builds carry none: `has_lcp()` is `false` and the contraction
  / parent operations return `LcpNotBuilt`. `BidirFmIndex::has_lcp()` is true only when
  both halves carry one; use `fwd().has_lcp()` / `rev().has_lcp()` for a single direction.
...
- **Breaking.** `FmIndexConfig` gains `build_lcp: bool` (default `false`); exhaustive
  struct literals must supply it. Set it to `true` only when the cursor contraction /
  parent operations are needed; CPU-built indexes then grow by ~2.7 bytes per base per
  half. GPU construction never builds the LCP.
